...
app.post('/create-organisation', authenticateAndAuthorize(['user', 'admin', 'owner']), async (req, res) => {
    const { organisationName } = req.body; // Extract organisationName from request
    ...
    try {
        // Insert new organisation into the organisation table
        const organisationInsertQuery = `
            INSERT INTO organisation (organisation, user_id, organisation_secret)
            VALUES ($1, $2, gen_random_uuid()) -- Generate a unique UUID for organisation_secret
            RETURNING *;
        `;
        const organisationInsertValues = [organisationName, userId];
        
        const organisationResult = await pool.query(organisationInsertQuery, organisationInsertValues);

        console.log('New organisation created:', organisationResult.rows[0]);

        const newOrganisation = organisationResult.rows[0];
        ...
        // Check for specific database errors like permission issues
        if (error.code === '42501') {
            return res.status(403).json({ error: 'Permission denied to create organisation' });
        }
...
app.get('/list-organisations', authenticateAndAuthorize(['user', 'admin', 'owner']), async (req, res) => {
    ...
    try {
        // Query to fetch all organisations associated with the user
...
app.post('/create-dictionary', authenticateAndAuthorize(['admin', 'owner', 'user']), async (req, res) => {
    ...
    // Construct the data object where name is the key and actions are the values
    const data = { [name]: actions };

    try {
        const result = await pool.query(
            'INSERT INTO dictionaries (name, data) VALUES ($1, $2) RETURNING *',
            [name, JSON.stringify(data)] // Ensure the data is converted to a JSON string
        );
